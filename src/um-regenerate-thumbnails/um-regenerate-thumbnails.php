...
if ( ! function_exists( 'um_extended_regenerate_thumbs_loading_allowed' ) ) {
	/**
	 * Don't allow to run the plugin when  Ultimate Member plugin is not active/installed
	 *
	 * @since 1.0.0
	 */
	function um_extended_regenerate_thumbs_loading_allowed() {
		...
		if ( ! is_plugin_active( 'ultimate-member/ultimate-member.php' ) ) {
			...
			add_action( 'admin_notices', 'um_extended_regenerate_thumbs_ultimatemember_requirement_notice' );
	...
	if ( ! function_exists( 'um_extended_regenerate_thumbs_ultimatemember_requirement_notice' ) ) {
		/**
		 * Display the notice after activation
		 *
		 * @since 1.0.0
		 */
		function um_extended_regenerate_thumbs_ultimatemember_requirement_notice() {
	...
	if ( um_extended_regenerate_thumbs_loading_allowed() === false ) {
		return;
	}
 ...
 * @since 1.0.0
 *
 * @return UM_Extended_Regenerate_Thumbnails\Core
 */
function um_extended_regenerate_thumbs_plugin() {
	/**
	 * Load core class
	 *
	 * @var $core
	...
	if ( ! isset( $core ) ) {
		$core = new \UM_Extended_Regenerate_Thumbnails\Core();
	}
