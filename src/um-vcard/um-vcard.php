...
if ( ! function_exists( 'um_extended_vcard_loading_allowed' ) ) {
	/**
	 * Don't allow to run the plugin when  Ultimate Member plugin is not active/installed
	 *
	 * @since 1.0.0
	 */
	function um_extended_vcard_loading_allowed() {
	...
	if ( ! function_exists( 'um_extended_vcard_ultimatemember_requirement_notice' ) ) {
		/**
		 * Display the notice after activation
		 *
		 * @since 1.0.0
		 */
		function um_extended_vcard_ultimatemember_requirement_notice() {

			echo '<div class="notice notice-warning"><p>';
			printf(
				wp_kses( /* translators: %1$s - The Ultimate Member requires the latest version. */
					__( 'The Ultimate Member - VCard requires the latest version of <a href="%1$s" target="_blank" rel="noopener noreferrer">Ultimate Member</a> plugin to be installed &amp; activated.', 'um-extended' ),
					array(
						'a'      => array(
							'href'   => array(),
							'target' => array(),
	...
	if ( um_extended_vcard_loading_allowed() === false ) {
		return;
	}
 ...
 * @since 1.0.0
 *
 * @return UM_Extended_vcards\Core
 */
function um_extended_vcard_plugin() {
	/**
	 * Load core class
	 *
	 * @var $core
	...
	if ( ! isset( $core ) ) {
		$core = new \UM_Extended_Vcard\Core();
	}
