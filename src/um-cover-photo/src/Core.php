...
namespace UM_Extended_Cover_Photo;
...
class Core {
	...
	public function __construct() {
		...
		add_action( 'um_after_user_account_updated', array( $this, 'set_cover_photo' ), 1, 2 );
		add_action( 'um_registration_set_extra_data', array( $this, 'set_cover_photo' ), 1, 2 );
		add_action( 'template_redirect', array( $this, 'set_temp_user_id' ) );
		add_action( 'wp_footer', array( $this, 'cover_photo_script' ) );
		add_action( 'wp_ajax_um_remove_file', array( $this, 'delete_cover_photo_from_account' ), 1 );
	...
	public function predefined_fields( $arr ) {
		...
		$arr['register_cover_photo'] = array(
			'title'       => __( 'Cover Photo', 'um-extended' ),
			'metakey'     => 'register_cover_photo',
			'type'        => 'image',
			'label'       => __( 'Change your cover photo', 'um-extended' ),
			'upload_text' => __( 'Upload profile cover here', 'um-extended' ),
	 ...
	 */
	public function cover_photo_script() {
	...
	public function delete_cover_photo_from_account() {
		...
		if ( isset( $_REQUEST['mode'] ) && in_array( $_REQUEST['mode'], array( 'account', 'profile' ), true ) ) {  // phpcs:ignore WordPress.Security.NonceVerification
			...
			if ( 'cover_photo' === $fname ) {
				...
				UM()->common()->users()->delete_photo( get_current_user_id(), 'register_cover_photo' );
