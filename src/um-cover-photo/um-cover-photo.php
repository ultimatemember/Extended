...
if ( ! function_exists( 'um_extended_coverphoto_loading_allowed' ) ) {
	/**
	 * Don't allow to run the plugin when  Ultimate Member plugin is not active/installed
	 *
	 * @since 1.0.0
	 */
	function um_extended_coverphoto_loading_allowed() {
		...
		if ( ! is_plugin_active( 'ultimate-member/ultimate-member.php' ) ) {
			...
			add_action( 'admin_notices', 'um_extended_coverphoto_ultimatemember_requirement_notice' );
	...
	if ( ! function_exists( 'um_extended_coverphoto_ultimatemember_requirement_notice' ) ) {
		/**
		...
		function um_extended_coverphoto_ultimatemember_requirement_notice() {
			...
			printf(
				wp_kses( /* translators: %1$s - The Ultimate Member requires the latest version. */
					__( 'The Ultimate Member - Cover Photo requires the latest version of <a href="%1$s" target="_blank" rel="noopener noreferrer">Ultimate Member</a> plugin to be installed &amp; activated.', 'um-extended' ),
					array(
						'a'      => array(
							'href'   => array(),
							'target' => array(),
	...
	if ( um_extended_coverphoto_loading_allowed() === false ) {
		return;
	}
...
function um_extended_coverphoto_plugin() {
	...
	if ( ! isset( $core ) ) {
		$core = new \UM_Extended_Cover_Photo\Core();
	}
